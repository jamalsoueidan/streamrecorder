...
export function VideoModal({
  ...
  const basePath = `/my/${type}/${username}/video`;
  const queryString = searchParams.toString();

  useEffect(() => {
    const path = `${basePath}/${currentId}`;
    const localePath = locale === "en" ? path : `/${locale}${path}`;
    const url = `${localePath}${queryString ? `?${queryString}` : ""}`;
    window.history.replaceState({}, "", url);
  }, [currentId, basePath, queryString, locale]);
    ...
    >
      <Flex h="100%" justify="center" align="center" pos="relative">
        ...
        <PlayBack
          prevId={prevId}
          nextId={nextId}
          basePath={`/my/${type}/${username}/video`}
