...
// Check if profile::run task is completed
function isProfileCompleted(tasks: AiTask[]): boolean {
  return tasks.some(
    (task) => task.type === "profile::run" && task.state === "completed",
  );
}

// Parse task output to extract counts
function getTaskOutputInfo(
  task: Pick<AiTask, "type" | "state" | "output">,
): string | null {
...
function getTaskDuration(
  task: Pick<AiTask, "createdAt" | "updatedAt" | "state">,
): string | null {
...
export function AiRequestStatus({ aiRequest, follower }: Props) {
  ...
  const completedTasks = visibleTasks.filter((t) => t.state === "completed").length;
  const failedTasks = visibleTasks.filter((t) => t.state === "failed").length;
  const processingTasks = visibleTasks.filter((t) => t.state === "processing").length;
  ...
  const stateIcons: Record<string, React.ReactNode> = {
    ...
    processing: <IconLoader size={16} className="animate-spin" color="var(--mantine-color-blue-6)" />,
