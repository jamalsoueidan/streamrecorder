...
import { encodePath } from "@/app/lib/media-url";
...
function buildVideoRanges(sources: Source[]) {
  ...
  const host = baseUrl ? new URL(baseUrl).hostname.replace(/^www\./, "") : "livestreamrecorder.com";
  const mediaHost = `https://media.${host}`;
  ...
  for (const source of sources) {
    ...
    ranges.push({
      ...
      path: `${mediaHost}/${bucket}${encodePath(source.path || "")}`,
